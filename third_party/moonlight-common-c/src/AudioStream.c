...
#ifdef __PPU__
    // PS3 libnet max SO_RCVBUF is ~64KB. Request the maximum accepted value.
    // bindUdpSocket()'s fallback loop will step down if even this is rejected.
    int audioRcvBufSize = 64 * 1024;
#else
    int audioRcvBufSize = 0;
#endif
    rtpSocket = bindUdpSocket(RemoteAddr.ss_family, &LocalAddr, AddrLen, audioRcvBufSize, SOCK_QOS_TYPE_AUDIO);

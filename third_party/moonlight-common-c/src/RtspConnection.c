...
static bool initializeRtspRequest(PRTSP_MESSAGE msg, char* command, char* target)
{
    ...
    if (!addOption(msg, "CSeq", sequenceNumberStr) ||
        ...
        !addOption(msg, "User-Agent", "Moonlight") ||
...
static char* sealRtspMessage(PRTSP_MESSAGE request, int* messageLen) {
    ...
    if (serializedMessage == NULL) {
        Limelog("serializeRtspMessage failed\n");
        return NULL;
    }
...
static bool unsealRtspMessage(char* rawMessage, int rawMessageLen, PRTSP_MESSAGE response) {
    ...
    if (rawMessageLen == 0) {
        Limelog("Server closed RTSP connection without response\n");
        return false;
    }
...
static bool transactRtspMessageTcp(PRTSP_MESSAGE request, PRTSP_MESSAGE response, int* error) {
    ...
    *error = -1;
    ...
    if (serializedMessage == NULL) {
        closeSocket(sock);
        sock = INVALID_SOCKET;
        Limelog("sealRtspMessage failed\n");
        return ret;
    }
    ...
    enableNoDelay(sock);
    Limelog("Sending RTSP message (len: %d):\n%.*s\n", messageLen, messageLen, serializedMessage);
    ...
    Limelog("RTSP message sent, waiting for response...\n");
...
static bool requestOptions(PRTSP_MESSAGE response, int* error) {
    ...
    *error = -1;
    ...
    // Use '*' for OPTIONS request target as per RTSP spec for global options
    ret = initializeRtspRequest(&request, "OPTIONS", "*");
    if (ret) {
...
int performRtspHandshake(PSERVER_INFORMATION serverInfo) {
    ...
    useEnet = false; // Forced to false for Sunshine compatibility and better debugging on PS3
    currentSeqNumber = 1;
    hasSessionId = false;
    ...
    // Always use the real remote address for the RTSP URL and Host header.
    // The previous 0.0.0.0 hack for high quality audio on GFE was rejected by Sunshine.

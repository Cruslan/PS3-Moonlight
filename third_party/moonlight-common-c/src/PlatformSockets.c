...
#if defined(__PPU__)
    // PS3's libnet returns ENOBUFS (105) when the kernel mbuf pool is
    // temporarily exhausted under high-bitrate UDP traffic.  Treat it
    // as transient — just like ICMP port-unreachable — and keep reading.
    if (err < 0 && LastSocketError() == ENOBUFS) {
        ps3_enobufs_count++;
    }
    } while (err < 0 && (LastSocketError() == ECONNREFUSED || LastSocketError() == ENOBUFS));
...
SOCKET bindUdpSocket(int addressFamily, struct sockaddr_storage* localAddr, SOCKADDR_LEN addrLen, int bufferSize, int socketQosType) {
    ...
    if (bufferSize != 0) {
        ...
        for (;;) {
            // PS3 libnet only accepts even numbers. Ensure bufferSize is even.
            bufferSize &= ~1;
...
#if defined(LC_DEBUG) || defined(__PPU__)
        // Always log on PS3 to diagnose the actual accepted buffer size
        if (err == 0) {
...
int setSocketNonBlocking(SOCKET s, bool enabled) {
    ...
    int val = enabled ? 1 : 0;
    return setsockopt(s, SOL_SOCKET, SO_NBIO, (char*)&val, sizeof(val));
...
#ifdef __PPU__
    // PS3 libnet requires protocol 0 for default
...
#elif defined(TCP_MAXSEG) && !defined(__PPU__)
...
int sendMtuSafe(SOCKET s, char* buffer, int size) {
    ...
    while (bytesSent < size) {
        ...
        /* Hex dump — commented out, enable for deep debugging:
        {
            int i;
            Limelog("Sent %d bytes:\n", bytesToSend);
            for (i = 0; i < bytesToSend; i++) {
                Limelog("%02X ", (unsigned char)buffer[bytesSent + i]);
                if ((i + 1) % 16 == 0) Limelog("\n");
            }
            Limelog("\n");
        }
...
int resolveHostName(const char* host, int family, int tcpTestPort, struct sockaddr_storage* addr, SOCKADDR_LEN* addrLen)
{
    ...
    {
        char resolvedHost[URLSAFESTRING_LEN];
        addrToUrlSafeString((struct sockaddr_storage*)res->ai_addr, resolvedHost, sizeof(resolvedHost));
        Limelog("getaddrinfo(%s) resolved to: %s\n", host, resolvedHost);
    }
...
#else
#if defined(LC_POSIX) && !defined(LC_CHROME) && !defined(__PS3__)

...
#if defined(LC_WINDOWS) || defined(__WIIU__) || defined(__3DS__)
    return 0;
#elif defined(__vita__)
    return;
#elif defined(__PPU__)
    /* PS3: Must call sysThreadExit() - returning from thread function
       causes a crash to 0xbadadd00 sentinel address in PSL1GHT */
    sysThreadExit(0);
#else
    return NULL;
#endif
...
int PltCreateMutex(PLT_MUTEX* mutex) {
    ...
    int err = sysMutexCreate(mutex, &attr);
    if (err != 0) {
        return err;
    }
#else
...
#elif defined(__PPU__)
    u64 retval;
    sysThreadJoin(thread->thread, &retval);
#else
...
#elif defined(__PPU__)
    {
        // PS3 PPC64 ABI uses large stack frames (~48+ bytes each for LR save,
        // backchain, parameter area). The VideoRecv call chain is deep:
        // recv → decrypt → RTP queue → FEC (Reed-Solomon) → depacketize → video callbacks.
        // 64KB (0x10000) overflows; 256KB (0x40000) provides sufficient headroom.
        
        // Increase priority for network-critical threads
        int priority = 1001; // Default
        if (name && (strcmp(name, "VideoRecv") == 0)) {
            priority = 100; // Ultra-high priority for networking
        } else if (name && (strcmp(name, "VideoDec") == 0)) {
            priority = 600; // High priority for decoding
        } else if (name && (strcmp(name, "VideoPing") == 0)) {
            priority = 500; // Ping must be fast but not stall-critical
        }

        int err = sysThreadCreate(&thread->thread, (void (*)(void *))ThreadProc, ctx, priority, 0x40000, THREAD_JOINABLE, (char*)name);
        if (err != 0) {
            free(ctx);
            return err;
        }
...
int PltCreateConditionVariable(PLT_COND* cond, PLT_MUTEX* mutex) {
    ...
    int err = sysCondCreate(cond, *mutex, &attr);
    if (err != 0) {
        return err;
    }
#else
...
uint64_t PltGetMicroseconds(void) {
    ...
    u64 sec, nsec;
    sysGetCurrentTime(&sec, &nsec);
    uint64_t now = sec * 1000000 + nsec / 1000;
    return now - start;
}

...
static bool sendMessageEnet(short ptype, short paylen, const void* payload, uint8_t channelId, uint32_t flags, bool moreData) {
    ...
    // Guard against use-after-teardown: peer/client may be NULL if
    // stopControlStream() runs between the 'stopping' check and here.
    ...
    if (encryptedControlStream) {
        ...
        // Re-check after acquiring the lock: stopControlStream() may have
        // destroyed peer/client while we were waiting for the mutex.
        if (peer == NULL || client == NULL || stopping) {
        ...
        // Re-check after acquiring the lock (same race as encrypted path)
        if (peer == NULL || client == NULL || stopping) {
            enet_packet_destroy(enetPacket);
            PltUnlockMutex(&enetMutex);
            return false;
...
int stopControlStream(void) {
    ...
    // Destroy ENet resources under lock to synchronize with InputSend
    // and any other thread that may be inside sendMessageEnet().
    PltLockMutex(&enetMutex);
    ENetPeer* peerToDisconnect = peer;
    ENetHost* clientToDestroy = client;
    peer = NULL;
    client = NULL;
    PltUnlockMutex(&enetMutex);

    if (peerToDisconnect != NULL && clientToDestroy != NULL) {
        gracefullyDisconnectEnetPeer(clientToDestroy, peerToDisconnect, CONTROL_STREAM_LINGER_TIMEOUT_SEC * 1000);
    }
    if (clientToDestroy != NULL) {
        enet_host_destroy(clientToDestroy);
    }
...
#ifdef __PPU__
        // PS3 libnet's recvfrom() does not fill the source address buffer,
        // so peer->address gets overwritten with zeros during the handshake.
        // Restore it from remoteAddress which is the known server address.

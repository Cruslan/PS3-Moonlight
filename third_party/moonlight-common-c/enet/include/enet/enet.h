...
enum
{
   ...
   // PS3 libnet rejects SO_RCVBUF/SO_SNDBUF values above ~64 KB with EINVAL.
   // ENet sets this in a single setsockopt() call with no fallback loop, so
   // the request must succeed on the first try.  32 KB is well within the
   // limit and prevents ENOBUFS (105) on the control stream socket.
   ENET_HOST_RECEIVE_BUFFER_SIZE          = 32 * 1024,
   ENET_HOST_SEND_BUFFER_SIZE             = 32 * 1024,
#else
   ENET_HOST_RECEIVE_BUFFER_SIZE          = 256 * 1024,

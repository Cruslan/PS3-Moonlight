...
/* PS3 / PSL1GHT - use NO_MSGAPI, poll from net/poll.h */
#ifndef NO_MSGAPI
#define NO_MSGAPI 1
#endif
#ifndef HAS_POLL
...
{
#ifdef __PPU__
    /* PS3: sockaddr_storage is 128 bytes but libnet only accepts sizeof(sockaddr_in)=16 */
...
#ifdef __PPU__
            /* PS3: use setsockopt(SO_NBIO) - ioctl(FIONBIO) may not be available in libnet */
            result = setsockopt (socket, SOL_SOCKET, SO_NBIO, (char *) & value, sizeof(int));
                  ...
                  size_t bufferCount)
{
    ...
    if (sentLength < 0) {
        ...
        printf("[ENET-DBG] sendto failed: errno=%d sock=%d len=%u addrlen=%d family=%d port=%d\n",
            errno, (int)socket, (unsigned)sendLength, (int)peerAddress->addressLength,
            (int)sin->sin_family, (int)ntohs(sin->sin_port));
    }
#endif
    if (bufferCount > 1)
      free(sendBuffer);
                     ...
                     size_t bufferCount)
{
    ...
    /* PS3: sockaddr_storage is 128 bytes but libnet only accepts sizeof(sockaddr_in)=16 */
    {
        /* Save current address in case recvfrom doesn't fill it (PS3 libnet quirk) */
        struct sockaddr_storage savedAddr;
        socklen_t savedLen = peerAddress->addressLength;
        memcpy(&savedAddr, &peerAddress->address, sizeof(savedAddr));

        peerAddress->addressLength = sizeof(struct sockaddr_in);
        recvLength = recvfrom(socket, buffers[0].data, buffers[0].dataLength, MSG_NOSIGNAL,
            (struct sockaddr *)&peerAddress->address, &peerAddress->addressLength);

        /* If recvfrom didn't fill address (family=0), restore the saved address */
        if (peerAddress->address.ss_family == 0) {
            memcpy(&peerAddress->address, &savedAddr, sizeof(savedAddr));
            peerAddress->addressLength = savedLen;
        }
    }
#else

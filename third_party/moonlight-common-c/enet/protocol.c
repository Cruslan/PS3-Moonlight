...
enet_protocol_handle_incoming_commands (ENetHost * host, ENetEvent * event)
{
    ...
    {
#ifdef __PPU__
       /* PS3: recvfrom() never fills source address, so receivedPeerAddress is always zeroed.
        * Do NOT overwrite the peer's known address with zeros. */
       if (host->receivedPeerAddress.address.ss_family != 0) {
